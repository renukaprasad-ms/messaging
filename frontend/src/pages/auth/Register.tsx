...
const Register = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.data) navigate(homePath(response.data), { replace: true })
    } catch (err) {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm leading-6 text-slate-500">
        Start managing customer conversations and campaigns from one place.
      </p>
      ...
      <form className="mt-7 space-y-4" onSubmit={handleSubmit}>
        ...
        <PasswordField value={form.password} onChange={(value) => updateField('password', value)} />
        <label className="block">
        ...
        <button
          ...
          className="mt-2 h-12 w-full rounded-xl bg-indigo-500 text-sm font-semibold text-white shadow-lg shadow-indigo-100 hover:bg-indigo-600 disabled:cursor-not-allowed disabled:bg-indigo-300"
        >
